...
#запуск и выполнение потока
    def run(self):
        ...
        if self._simple_brute() is not None:
            print(f"thread '{self.name}' ended, result: {self.word}")
        self.__is_ready = False
    ...
    def wait_timer_or_event(self):
        ...
        while self.__active_thread.is_ready():
            if time.time_ns() - start >= TICK:
                ...
                print("Ta ", active_thread.name)
                break
        return


threads = [BruteThread("First()", WORDS[1]), BruteThread("Second()", WORDS[2]), BruteThread("Third()", WORDS[0])]
...
def on_hotkey():
    if wait_queue.qsize() >= 1:
        ...
        print("key is pressed, processing ", active_thread.name)
    else:
        if Planner.get_active_thread() is not None:
            ...
            print("key is pressed, waiting ", active_thread)
    return
